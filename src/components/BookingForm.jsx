...
import { DURATIONS, calculateTotalPrice, FIXED_ADVANCE, getHourPrice } from '../utils/constants';
import { formatCurrency } from '../utils/helpers';
import { User, Phone, Clock, Tag, CreditCard, Loader2, Moon, Sun } from 'lucide-react';
...
export default function BookingForm({ court, date, startHour, onSuccess, onCancel }) {
  ...
  // Calculate dynamic price based on court and hour
  const totalPrice = useMemo(
    () => calculateTotalPrice(court, startHour, duration),
    [court, startHour, duration]
  );
  const advanceAmount = FIXED_ADVANCE;
  const remaining = totalPrice - advanceAmount;
  const hourPrice = getHourPrice(court, startHour);
  const isNightTime = startHour >= 18;
  ...
  const handleSubmit = (e) => {
    ...
    // VIP code: skip payment
    if (vipCode.trim()) {
      submitVipReservation();
      return;
    }
  ...
  const submitVipReservation = async () => {
    setLoading(true);
    try {
      ...
      if (data.success) {
        addNotification('Reserva VIP realizada correctamente', 'success');
  ...
  const handlePaymentSuccess = (reservation) => {
    ...
    addNotification('Pago aprobado, reserva confirmada', 'success');
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className={`animate-fade-in p-6 rounded-3xl border-2 ${
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <span className={`text-sm font-semibold px-3 py-1 rounded-full flex items-center gap-1 ${
            ...
            {isNightTime ? <Moon size={12} /> : <Sun size={12} />}
        ...
        <div className={`mb-4 p-3 rounded-xl flex items-center justify-between ${
          isNightTime
            ...
            : darkMode ? 'bg-energy-900/30 border border-energy-800' : 'bg-energy-50 border border-energy-200'
        }`}>
          <div>
            <p className={`text-xs font-semibold ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Tarifa {isNightTime ? 'nocturna (6pm en adelante)' : 'diurna (antes 6pm)'}
            </p>
            <p className={`text-sm font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
              {formatCurrency(hourPrice)} / hora en {court}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Código VIP (opcional, sin pago)"
              className={`${inputClass} pl-11`}
            />
          </div>
      ...
      {showPayment && (
        <PaymentModal
          bookingData={{ name: name.trim(), phone: phone.trim(), court, date, startHour, duration }}
